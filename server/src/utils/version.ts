...
export class Version implements IVersion {
	...
	constructor(public major: number, public minor: number = 0, public patch: number = 0) {}
	...
	static fromString(version: string): Version {
		...
		if (matchResult) {
			const {
				...
				minor = '0',
				patch = '0'
			} = matchResult.groups as { [K in keyof IVersion]: string };
	...
	private compare(version: Version): [number, VersionType] {
		for (const [index, key] of this.types.entries()) {
			const diff = this[key] - version[key];

			if (diff !== 0) {
				return [diff > 0 ? 1 : -1, (VersionType.MAJOR - index) as VersionType];
			}
		}
